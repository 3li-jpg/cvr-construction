...
const TextAnimateBase = ({
  ...
  const finalVariants = prefersReducedMotion
    ? {
        ...
        item: {
          hidden: { opacity: 1, filter: "blur(0px)", x: 0, y: 0, scale: 1 },
          show: { opacity: 1, filter: "blur(0px)", x: 0, y: 0, scale: 1 },
          exit: { opacity: 1, filter: "blur(0px)", x: 0, y: 0, scale: 1 },
        },
      }
    : variants
      ? {
  ...
  return (
    <AnimatePresence mode="popLayout">
      <MotionComponent
        ...
        exit="exit"
        className={joinClassNames("whitespace-pre-wrap", className)}
        viewport={{ once }}
